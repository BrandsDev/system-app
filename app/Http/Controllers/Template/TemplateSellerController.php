...
class TemplateSellerController extends Controller
{
    /**
    ...
    {
        return view('administration.template.seller.new-seller');
    }
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        if ($request->hasFile('image')) {
            ...
            $request->file('image')->move(public_path('template/seller/image'), $image);
            $seller->image = $image;
        }

        if ($request->hasFile('og_image')) {
            $oGImage = $request->file('og_image')->getClientOriginalName();
            $request->file('og_image')->move(public_path('template/seller/image/og'), $oGImage);
            $seller->og_image = $oGImage;
        }

        if ($request->hasFile('cover')) {
            $coverImage = $request->file('cover')->getClientOriginalName();
            $request->file('cover')->move(public_path('template/seller/image/cover'), $coverImage);
        ...
        return redirect(RouteServiceProvider::TemplateSeller);
    }
    ...
    public function show(Request $sellerDetail)
    {
        ...
        return view('administration.template.seller.manage-sellers', ['sellers' => $sellers]);
    }
    ...
    {
        $sellers = TemplateSeller::where('slug', $slug)->firstOrFail();

        return view('frontend.template.template-detail', ['sellers' => $sellers]);
    }
    ...
    {
        $seller = TemplateSeller::findOrFail($id);

        return view('administration.template.seller.edit-seller', ['seller' => $seller]);
    }
    ...
    public function update(Request $request, $id): RedirectResponse
    {
        ...
        if ($seller) {
            ...
            if ($newImage) {
                ...
                $request->image->move(public_path('template/seller/image'), $newImageName);
            ...
            if ($newOGImage) {
                ...
                $request->og_image->move(public_path('template/seller/image/og'), $oGImage);
            ...
            if ($newCover) {
                ...
                $request->cover->move(public_path('template/seller/image/cover'), $coverImage);
    ...
    {
        TemplateSeller::where('id',$id)->delete();
